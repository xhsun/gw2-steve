...
public class WalletDB extends Database<WalletModel> {
	 ...
	 */
	List<WalletModel> getAllByCurrency(int id) {
	 ...
	 * @return list of wallets | empty on not find
	 */
	List<WalletModel> getAllByAPI(String key) {
	...
	//parse get result
	@Override
	protected List<WalletModel> __parseGet(Cursor cursor) {
		List<WalletModel> currencies = new ArrayList<>();
		if (cursor.moveToFirst())
			while (!cursor.isAfterLast()) {
				WalletModel currency = new WalletModel();
