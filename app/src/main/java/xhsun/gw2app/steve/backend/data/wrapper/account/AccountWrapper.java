package xhsun.gw2app.steve.backend.data.wrapper.account;
...
public class AccountWrapper {
	 ...
	 */
	public AccountModel addAccount(String api) throws IllegalArgumentException {
		...
		try {
			...
			if (database.createAccount(api, id, name, world_id, world, access))
				return new AccountModel(api, id, name, world_id, world, access, true);
	 ...
	 */
	public boolean markInvalid(AccountModel account) {
		boolean isSuccess;
		String api;
		if (account == null || (api = account.getAPI()) == null || "".equals(api)) {
			Timber.d("Did not provide an API key");
	 ...
	 */
	public AccountModel get(String api) {
	 ...
	 */
	public List<AccountModel> getAll(Boolean state) {
...
//	}
	...
	public void updateAccounts() {
		List<AccountModel> accounts = getAll(true);//no point check inaccessible account
		for (AccountModel info : accounts) {
