...
public class AccountDB extends Database<AccountModel> {
	 ...
	 */
	AccountModel getUsingAPI(String api) {
	 ...
	 * @return account detail | null if not find
	 */
	AccountModel getUsingGUID(String id) {
	 ...
	 * @return list of all accounts | empty if not find
	 */
	List<AccountModel> getAllWithState(boolean isValid) {
	...
	protected List<AccountModel> __parseGet(Cursor cursor) {
		...
		if (cursor.moveToFirst())
			while (!cursor.isAfterLast()) {
				AccountModel account = new AccountModel(cursor.getString(cursor.getColumnIndex(API)),
